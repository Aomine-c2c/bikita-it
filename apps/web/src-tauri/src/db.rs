...
impl Database {
    pub fn new(app_dir: PathBuf) -> std::result::Result<Self, Box<dyn std::error::Error>> {
    ...
    fn migrate(&self) -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        conn.execute_batch(r#"
            ...
            CREATE TABLE IF NOT EXISTS inventory_items (
                ...
                current_meter_mark INTEGER NOT NULL DEFAULT 0,
            ...
            BEGIN
              ...
              SET assignee_id = NEW.employee_id, status = 'IN_USE', updated_at = datetime('now')
            ...
            BEGIN
              ...
              UPDATE hardware_assets 
              SET assignee_id = NULL, status = 'AVAILABLE', updated_at = datetime('now')
              WHERE id = NEW.asset_id;
            END;
            ...
            BEGIN
              ...
              UPDATE hardware_assets 
              SET location_id = NEW.location_id, status = 'INSTALLED', updated_at = datetime('now')
              WHERE id = NEW.asset_id;
            END;
            ...
            BEGIN
              INSERT INTO timeline_events (id, entity_type, entity_id, event_type, description)
              VALUES (lower(hex(randomblob(16))), 'INSTALLATION', NEW.asset_id, 'REMOVED', 'Asset uninstalled from location.');
              
              UPDATE hardware_assets 
              SET location_id = NULL, status = 'AVAILABLE', updated_at = datetime('now')
              WHERE id = NEW.asset_id;
            END;

            -- Triggers for REPAIRS
            CREATE TRIGGER IF NOT EXISTS repair_insert_trigger
            AFTER INSERT ON repairs
            BEGIN
              INSERT INTO timeline_events (id, entity_type, entity_id, event_type, description)
              VALUES (lower(hex(randomblob(16))), 'MAINTENANCE', NEW.hardware_id, 'REPAIR_STARTED', 'Asset sent for repair: ' || NEW.description);
              
              UPDATE hardware_assets 
              SET status = 'IN_REPAIR', updated_at = datetime('now')
              WHERE id = NEW.hardware_id;
            END;

            CREATE TRIGGER IF NOT EXISTS repair_update_trigger
            AFTER UPDATE ON repairs
            WHEN OLD.status != 'COMPLETED' AND NEW.status = 'COMPLETED'
            BEGIN
              INSERT INTO timeline_events (id, entity_type, entity_id, event_type, description)
              VALUES (lower(hex(randomblob(16))), 'MAINTENANCE', NEW.hardware_id, 'REPAIR_COMPLETED', 'Asset repair completed.');
        ...
        // Ensure the new current_meter_mark column exists in existing DBs
        let _ = conn.execute("ALTER TABLE inventory_items ADD COLUMN current_meter_mark INTEGER NOT NULL DEFAULT 0;", []);
        ...
        // For existing databases, attempt to add the updated_at column to tables that were missing it in earlier schema versions.
        ...
        let _ = conn.execute("ALTER TABLE inventory_items ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''", []);
        let _ = conn.execute("ALTER TABLE locations ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''", []);
        let _ = conn.execute("ALTER TABLE connected_devices ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''", []);
