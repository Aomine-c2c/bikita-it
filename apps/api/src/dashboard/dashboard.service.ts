...
export class DashboardService {
  ...
  async getStats() {
    try {
      ...
      const [totalHardware, atRiskHardware, lowStockItemsCount, activeNetworkDevices] = await Promise.all([
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        transactionTrend.push({ day: dayName, received: intakeCount, issued: issueCount });
      }
      ...
      return {
        ...
        activeRepairs: activeRepairs.map(r => ({
          ...
          eta: r.estimatedCompletion ? r.estimatedCompletion.toLocaleDateString() : 'TBD'
        })),
